...
/*:
 ...
 Using a symmetric key, all communicating parties must posses the same key in order to be able to encrypt or decrypt. It is important to mention at this point that messages are being encrypted and decrypt using the exact same key in this scheme.
 ...
 - Theoretically, most cryptographic algorithms are solveable. We have to differentiate between mathematic theory and computer scientific practice - because in practice, they currently are not solveable (unless you have milleniums of time to spend on solving them). The processing power of computers, however evolves relatively quickly. By more powerful machines becoming available, the list of algorithms considered secure thus changes. Another interesting topic to keep in mind is quantum computers. In theory, they can solve many mathematic problems cryptographic algorithms rely on with not problems whatsoever. We'll see what them eventually becoming available in the future will mean for today's cryptographic world.
